...
'''
THIS FILE TAKES A PICKLED ASSIGNMENT AND TURNS IT INTO JSON.
USED IF PICKLE IS NOT WORKING.
'''

STATE = "NC"
ENSEMBLE_NAME = "Ensemble_5_Steps_100"
...
RECOVERY_FOLDER = f"{os.path.dirname(os.path.abspath(__file__))}/JSON_{ENSEMBLE_NAME}"
...
for filename in os.listdir(ENSEMBLE_DIR):
    ...
    cur_dp_dict = current_district_plan.assignment.to_dict()
    with open(f"./JSON_{ENSEMBLE_NAME}/{filename}.json", 'w') as fp:
        json.dump(cur_dp_dict, fp)    